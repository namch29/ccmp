...
class ProjectCCMP(app_manager.RyuApp):
    ...
    def _switch_features_handler(self, ev):
        ...
        #src=6
        #dest=9
        #paths_weight = self.get_paths(read_file_path, src, dest)
        #weight=[]
        #flows=[]
        #print "Available paths from ", src, " to ", dest, " : ", paths
        #print paths
        #for i in xrange(0,len(paths_weight)):
			#weight.append(float(paths_weight[i][0]))
			#flows.append([int(x) for x in paths_weight[i][1:]])
        #print "weight=",weight,"flow",flows
        #print len (flows)
