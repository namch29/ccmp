                       ...
                       uint32_t basis)
{
    ...
    //******BEGIN*****ccmp modification****
	
    uint32_t total_weight = 0, sum = 0;
    ...
    uint32_t chosen_one = (uint32_t)(rand_num * (float)total_weight);
    
    VLOG_INFO("V2\ttotal_weight: %d =======chosen_one:%d", total_weight, chosen_one);
    
    LIST_FOR_EACH (bucket, list_node, buckets) {
        if (bucket_is_alive(ctx, bucket, 0)) {
            //uint32_t score =
              //  (hash_int(bucket->bucket_id, basis) & 0xffff) * bucket->weight;
            if (chosen_one > sum && chosen_one <=(sum + bucket->weight)) {
                best_bucket = bucket;
