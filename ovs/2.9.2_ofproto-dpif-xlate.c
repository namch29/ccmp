...
//The first parameter to HASH_ADD_INT is the hash table, and the second parameter is the name of the key field. 
//Here, this is id. The last parameter is a pointer to the structure being added.
    return p;
}


static const struct ofputil_bucket *
ccmp_roundRobin_bucket(const struct xlate_ctx *ctx,
    ...
    LIST_FOR_EACH (bucket, list_node, &group->up.buckets) {
        if(wrr_state->chosen_bucket==bucket_index && bucket_is_alive(ctx, bucket, 0)){
            ...
            if((wrr_state->counter % bucket->weight) == 0){
                ...
                wrr_state->chosen_bucket = (wrr_state->chosen_bucket + 1) % ovs_list_size(&group->up.buckets);
                wrr_state->counter = 0;
            }
            VLOG_INFO("===============Using weighted_rr_switching============= Bucket weight:%d",bucket->weight);
            return bucket;
        }
...
static struct ofputil_bucket *
ccmp_probabilistic_bucket(const struct xlate_ctx *ctx,
                       const struct group_dpif *group)
{
	//struct ofputil_bucket *best_bucket = NULL;
    uint32_t weight_sum = 0, total_weight = 0;
    struct ofputil_bucket *bucket;
    ...
    float rand_num = ((float)(rand() % 10) + 1)/10;  
    uint32_t score = (uint32_t)(rand_num * (float)total_weight);
    //VLOG_INFO("25-07\tV3\ttotal_weight: %d \tChosen_one:%d", total_weight, score);
    
    LIST_FOR_EACH (bucket, list_node, &group->up.buckets) {
        if (bucket_is_alive(ctx, bucket, 0)) {
            weight_sum += bucket->weight;                        
            if (score <= weight_sum) {
                //VLOG_INFO("Bucket weight:%d", bucket->weight);
                return bucket;                
            }
        }
	}
...
group_best_live_bucket(const struct xlate_ctx *ctx,
                       ...
                       uint32_t basis)
{
	//return ccmp_probabilistic_bucket(ctx, group);
	return ccmp_roundRobin_bucket(ctx, group);
}
// ###CCMP END -  MODIFICATION ###
static bool
...
xlate_group_bucket(struct xlate_ctx *ctx, struct ofputil_bucket *bucket,
                   bool is_last_action)
{
    VLOG_INFO("25-07-2018\tBucket weight: %d \t action:%d", bucket->weight,bucket->bucket_id);
                           ...
                           bool is_last_action)
{
    ...
    ctx->xout->slow |= SLOW_ACTION;
    flow_mask_hash_fields(&ctx->xin->flow, wc, NX_HASH_FIELDS_ETH_SRC);
    //END------------ccmp modification    
...
{
    struct ofputil_bucket *bucket;
    VLOG_INFO("V1+++++++++++++ selection method: xlate_dp_hash_select_group=============>>");
