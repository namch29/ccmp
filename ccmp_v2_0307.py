...
class ProjectCCMP(app_manager.RyuApp):
    ...
    def install_paths(self, datapathID, src, first_port, dst, last_port, ip_src, ip_dst, rev=False):
        ...
        if datapathID not in switches_in_paths: 
			print "\n===============switch is not in path=============\n"
			return
        
        
        dp = self.datapath_list[datapathID]
        ofp = dp.ofproto
        ...
        for path in paths_with_ports:
			if datapathID in path:
				in_port = path[datapathID][0]
				out_port = path[datapathID][1]
        ...
        for in_port in ports:
			...
			print " node:",datapathID," outport:", out_ports
			...
			if len(out_ports) > 1:
				...
				if (datapathID, src, dst) not in self.multipath_group_ids:
					group_new = True
					self.multipath_group_ids[
						datapathID, src, dst] = self.generate_openflow_gid()
				group_id = self.multipath_group_ids[datapathID, src, dst]
				print group_id
        ...
        print "paths_with_ports[0][src][1]", paths_with_ports[0][src][1]
        print "==============================\n"
    ...
    def _packet_in_handler(self, ev):
        ...
        if arp_pkt:
            ...
            if arp_pkt.opcode == arp.ARP_REPLY:
                ...
                print h1[0]
                print h2[0]
                self.install_paths(dpid, h1[0], h1[1], h2[0], h2[1], dst_ip, src_ip)
                self.install_paths(dpid, h2[0], h2[1], h1[0], h1[1], src_ip, dst_ip, rev = True) # reverse
                ...
                if dst_ip in self.arp_table:
                    ...
                    #print paths
                    self.install_paths(dpid, h1[0], h1[1], h2[0], h2[1], src_ip, dst_ip)
                    self.install_paths(dpid, h2[0], h2[1], h1[0], h1[1], dst_ip, src_ip,rev = True) # reverse
